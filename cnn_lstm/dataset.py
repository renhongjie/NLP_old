...
class DatasetParam(object):
    ...
    def __init__(self):
        ...
        self.max_sentence_len = 300     #句子最大词数
...
class Dataset(object):
    ...
    def _create_vocab(self, vocab_file):
        ...
        b = time.time()
        count = 0
        for root, _, fnames in os.walk(self.args.data_vocab_dir):
            for fname in fnames:
                ...
                with open(fpath, "r", encoding="utf-8") as rf:
                    ...
                    words = self.args.tokenizer(text)
                    for word in words:
                        word = word.strip()
                        assert word
                        if word in vocab:
    ...
    def _create_trainset(self, trainset_file, vocab_file, seg_sentence: bool) -> list:
        ...
        for i, cate in enumerate(self.args.cate_list):
            cate_dir = os.path.join(self.args.data_dir, cate)
            count = 0
            cate_set = []
            for root, _, fnames in os.walk(cate_dir):
                for fname in fnames:
                    fpath = os.path.join(root, fname)
                    count += 1
                    logging.info("count:%d, cate:%s, fname:%s" % (count, cate, fname))
                    with open(fpath, "r", encoding="utf-8") as rf:
                        text = rf.read().strip()
                        if seg_sentence:
                            text_size = len(text)
                            ...
                            word_ids = transform.text2ids(text, False)
                            if len(word_ids) > 0:
                                assert len(word_ids) == self.args.max_sentence_len
                                cate_set.append([i, word_ids])
            if len(cate_set) > 0:
